  ...
  padding: 16px;
        ...
        .btnRegister1
   {
    ...
    <Columns runat="server">
        <asp:BoundField DataField="youthID" HeaderText="youthID" InsertVisible="False" ReadOnly="True" SortExpression="youthID" />
    ...
    <Columns runat="server">
        <asp:BoundField DataField="adultID" HeaderText="adultID" InsertVisible="False" ReadOnly="True" SortExpression="adultID" />
        <asp:BoundField DataField="username" HeaderText="username" SortExpression="username" />        
        <asp:BoundField DataField="email" HeaderText="email" SortExpression="email" />
        <asp:BoundField DataField="lastUpdated" HeaderText="lastUpdated" SortExpression="lastUpdated" />
    </Columns>
...
<asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:manageUsers %>"  DeleteCommand= "DELETE FROM [Adult] WHERE [email]=@email;DELETE FROM [LoginInfo] WHERE [email]=@email" SelectCommand="SELECT [adultID], [username], [email], [lastUpdated] FROM [Adult]">
    <DeleteParameters>
        <asp:Parameter Name="email" />
    </DeleteParameters>
    </asp:SqlDataSource>
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:manageUsers %>"  DeleteCommand= "DELETE FROM [Youth] WHERE [email]=@email;DELETE FROM [LoginInfo] WHERE [email]=@email " SelectCommand="SELECT [youthID], [username], [email], [lastUpdated] FROM [Youth]">
    <DeleteParameters>
        <asp:Parameter Name="email" />
    </DeleteParameters>
    </asp:SqlDataSource>
